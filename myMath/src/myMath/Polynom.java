...
public class Polynom implements Polynom_able{
	...
	public Polynom (String s) {
		...
		int counter= 0;
		//realizing Monoms number by +,-
		for(int i=0;i<s.length();i++) {
			if (s.charAt(i)=='+'||s.charAt(i)=='-') {
				counter++;
			}
		}
		//saving +,- indexes in array
		int j[]=new int[counter];
		int b=0;
		...
		//split the string to pieces, cultivate them to Monoms,
		//and add them to our Polynom.
		b=0;
		int k=0;
		while(b<counter+1) {
			String z="";
			if(b==counter) { 
				if(s.charAt(j[b-1])=='-')z="-";
				z=z+s.substring(j[b-1]+1);
				b++;
			}
			else {
				if (b>0&&s.charAt(j[b-1])=='-')z= z+'-';
				z=z+s.substring(k,j[b++]);

			}
			// adding Monom to function
			Monom v=new Monom (z);
			f.add(v);	
			if (b<counter+1)k=j[b-1]+1;

		}
	}
/** calculate function in value x
 * @param x is the point in the Graph
 ...
 * @param p1 is the polynom which add
 * to our Polynom
 * 
 */
	 ...
	 * @param m1 is the Monom we added to polynom
	 */
	...
	public void add(Monom m1) {
		...
		this.a.sort(q);//sort the Polynom by x's power

	}

/**substrcut polynom
 * @param p1 is the Polynom we substruct with ours 
 */
	...
	/**multiply polynoms
	 * @param p1 is the polynom we Multiply 
	 */
 ...
 * @param p1 is the polynom we want to know if it equal to our polynom
 ...
 */
	...
	public boolean equals(Polynom_able p1) {
		...
		if(p.geta().size()!=this.a.size())return false;
...
/** fing x which f(x)=0 
 * @param x0 bound the line 
 * @param x1 bound the line
 * @param eps amount of calculation, precision volume
 ...
 */
	public String toString() {
		...
		Monom t=here.next();
		String s=t.get_coefficient()+"x^"+t.get_power()+" ";
		//first Monom 
		while(here.hasNext()) {
			Monom r=here.next();
 ...
 */
	///////////////////
	public ArrayList<Monom> geta() {
		return a;
	}
	/**
	 * 
	 * @param a- setting an polynom
	 */
	...
	public static void main(String args[]) {
		...
		Iterator <Monom> here = a.a.iterator();
		Monom b = new Monom (2,3);
		Monom c = new Monom (3,3);
		a.add(b);
		a.add(c);
		for(int i=0;i<a.a.size();i++){
		    System.out.println(a.a.get(i).get_coefficient());
		} */
		...
		Polynom z=new Polynom();
		Monom x=new Monom (2,1);
		Monom y= new Monom (2,2);
		z.add(x);
		z.add(y);
		...
		a.add(b);
		a.add(c);
        Polynom t=(Polynom) a.derivative();
        for(int i=0; i<a.a.size();i++) {
		...
		Iterator <Monom> dhere = bx.a.iterator();
		Monom z = new Monom (0,3); 
		...
		Polynom b = new Polynom();
		Iterator <Monom> dhere = b.a.iterator();
		...
		Polynom b = new Polynom();
		Monom z = new Monom (6,9);
		Monom s = new Monom (3,7);
		b.add(s);
		b.add(z);
		...
		a.add(b);
		a.add(c);
		System.out.println(a.root(-1,-4, 0.5));
		 */
		//checking function toString -clear!
		/*
	Polynom b = new Polynom();
	Monom z = new Monom (6,11);
	Monom s = new Monom (-3,10);
	b.add(s);
	b.add(z);
	System.out.println(b.toString());
		...
		;
		{}
		Polynom a=new Polynom("3x^1+4x^7+4-X^66");
